//this is the driver for the hash linear data structure 
...
void exportData(float insertArr[100], float searchArr[100], int insertCollisions[], int searchCollisions[], string fileName) // the purpose of this function is to take the arrays that have the runtimes and export them to a csv file to create the graphs 
{
    ...
    if(out_file.fail()) //if it failed, 
    {
        cout << "failed to open the output file" << endl;
        return;
    }
    out_file << "Iteration" << ',' << "Insert" << ',' << "Search" << ',' << "Insert Collisions" << ',' << "Search Collisions" << ',' << "Unit: µs" << endl; // titles 
    for (int i = 0; i < 100; i++) //insert all information into the out file 
    {
...
void searchFunction(float search[100], HashTable & table, int size, int intArr[], int range, int indx)
{
    ...
    {
        table.searchItem(intArr[randArr[i]]); //call search func
    }
    auto end = chrono::steady_clock::now(); //end
    search[indx] = chrono::duration_cast<chrono::microseconds>(end - start).count() / (100 * 1.0); // divide by 100 because it is average 
}
...
void insertFunction(float insert[100], float search[100], HashTable & table, int size, int intArr[], int totalCollisions[])
{
    ...
    for (int i = 0, j = 0; i < size; i++)
    {
        ...
        if ((i + 1) % 100 == 0 && i != 1)// we have reached the one hundreth node 
        {
            ...
            totalCollisions[j * 2 + 1] = table.getNumOfCollision();//these indices are for search collisions
            j++;
...
//collisions for each 100 inserts and searches
{
    for (int i = 0; i < 200; i++)
    {
        if (i % 2 == 0)//this is an insert value if the index is even 
        {
            if(i == 0)
            {
                insertCollisions[i] = totalCollisions[i]; //if it is the first, then it is merely the num of collisions that happened before the first/ 0th index 
            }
            else
            {
                insertCollisions[i/2] = totalCollisions[i] - totalCollisions[i-1]; //insert has 100 indices, so i/2; it is the number of collisions between current index and the prev index 
            }
        }
        else // it is a search collision index if it is odd because search indices are interwoven with inserts 
        {
            searchCollisions[i/2] = totalCollisions[i] - totalCollisions[i-1]; //current - previous = the difference =  num of collisions that happened during each search 
        }
    }
}
...
int main()
{
    ...
    if (processFile(dataSetB, intArr, size) == -1) // will return -1 if not succesful 
    {
        return 0;
    } 
    ...
    insertFunction(insert, search, table, size, intArr, totalCollisions);//this does the inserts. this then calls the search function every 100 inserts. 

    processCollisions(totalCollisions, insertCollisions, searchCollisions); // will process the collisions from the total collisions array 
    exportData(insert, search, insertCollisions, searchCollisions, "HashLinearTableGraphB.csv"); //export the data 
    ...
    insertFunction(insert, search, newTable, size, intArr, totalCollisions);//this does the inserts. this then calls the search function every 100 inserts. 
    processCollisions(totalCollisions, insertCollisions, searchCollisions);// will process the collisions from the total collisions array 
    exportData(insert, search, insertCollisions, searchCollisions, "HashLinearTableGraphA.csv"); // export the data 
